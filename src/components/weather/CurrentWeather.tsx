...
import { Thermometer, Droplets, Wind, Compass, Clock, RefreshCw } from 'lucide-react'
...
const getWeatherBadgeVariant = (condition: string): "default" | "secondary" | "destructive" | "outline" => {
  const lower = condition.toLowerCase()
  if (lower.includes('hujan') || lower.includes('badai')) return 'destructive'
  if (lower.includes('berawan') || lower.includes('mendung')) return 'secondary'
  if (lower.includes('cerah') || lower.includes('sunny')) return 'default'
  return 'outline'
}

export function CurrentWeather({ data, isLoading, onRefresh, className }: CurrentWeatherProps) {
    ...
    return (
      <Card className={cn("w-full", className)}>
        <CardContent className="flex items-center justify-center py-8">
          <div className="text-center space-y-2">
            <div className="text-muted-foreground">Tidak ada data cuaca tersedia</div>
            {onRefresh && (
  ...
  return (
    <Card className={cn("w-full bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-950/20 dark:to-indigo-950/20 border-blue-200 dark:border-blue-800", className)}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold text-blue-900 dark:text-blue-100">Cuaca Saat Ini</h2>
            <div className="flex items-center gap-2 text-sm text-blue-700 dark:text-blue-300">
              <Clock className="h-4 w-4" />
              <span>
                {isClient ? 
                  format(new Date(`${currentWeather.tanggal} ${currentWeather.jam}`), 'EEEE, dd MMMM yyyy • HH:mm', { locale: id }) :
          ...
          {onRefresh && (
            <Button variant="ghost" size="sm" onClick={onRefresh} className="text-blue-700 hover:text-blue-900 dark:text-blue-300 dark:hover:text-blue-100">
      ...
      <CardContent>
        ...
        <div className="text-center space-y-3 lg:hidden">
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="space-y-1">
              <div className={cn("text-4xl font-bold", getTemperatureColor(temperature))}>
                {temperature.toFixed(1)}°C
              </div>
              <Badge variant={getWeatherBadgeVariant(currentWeather.cuaca)} className="text-xs">
        ...
        <div className="hidden lg:grid lg:grid-cols-5 lg:gap-6 lg:items-center">
          ...
          <div className="col-span-2 text-center space-y-3">
            <div className="flex items-center justify-center gap-6">
              ...
              <div className="space-y-2">
                <div className={cn("text-5xl font-bold", getTemperatureColor(temperature))}>
                  {temperature.toFixed(1)}°C
                </div>
                <Badge variant={getWeatherBadgeVariant(currentWeather.cuaca)} className="text-sm">
                  {currentWeather.cuaca}
                </Badge>
              </div>
            </div>
          ...
          <div className="col-span-3 grid grid-cols-2 gap-4">
            ...
            <div className="bg-white/50 dark:bg-white/5 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="space-y-1">
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">Kelembapan</div>
            ...
            <div className="bg-white/50 dark:bg-white/5 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="space-y-1">
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">Kec. Angin</div>
                <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                  {windSpeedKmh.toFixed(1)} <span className="text-lg">km/h</span>
                </div>
            ...
            <div className="bg-white/50 dark:bg-white/5 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="space-y-1">
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">Arah Angin</div>
                <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
            ...
            <div className="bg-white/50 dark:bg-white/5 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="space-y-1">
                <div className="text-sm font-medium text-blue-700 dark:text-blue-300">Terasa Seperti</div>
                <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                  {(temperature + (humidity > 80 ? 2 : humidity > 60 ? 1 : 0)).toFixed(1)}°C
                </div>
              </div>
            </div>
          </div>
