...
export function DailyForecast({
  ...
  const dailyData = useMemo((): DailyData[] => {
    ...
    // Process each day
    const processedDays = Object.entries(groupedByDate).map(
      ([dateKey, hourlyData]) => {
        ...
        const dominantWeather =
          Object.entries(weatherCounts).sort(([, a], [, b]) => b - a)[0]?.[0] ||
          "Tidak diketahui";

        // Get the weather icon for the dominant condition
        const dominantWeatherItem = hourlyData.find(
          (item) => item.cuaca === dominantWeather
        );
        const weatherIcon = dominantWeatherItem?.iconUrl || "";

        // Get unique weather conditions for the day
        const weatherConditions = [
          ...new Set(hourlyData.map((item) => item.cuaca)),
        ];
        ...
        return {
          ...
          weatherIcon,
          weatherConditions,
          hourlyData: hourlyData.sort(
            (a, b) =>
              new Date(`${a.tanggal} ${a.jam}:00`).getTime() -
              new Date(`${b.tanggal} ${b.jam}:00`).getTime()
          ),
        };
      }
    );
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isToday &&
                  "bg-green-50 border-green-200 shadow-md ring-2 ring-green-100 dark:bg-green-950/20 dark:border-green-800 dark:ring-green-900/30",
            ...
            >
              ...
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="w-20 text-sm font-medium">
                  <div
                    className={cn(
                      isToday && "text-green-600 font-bold dark:text-green-400"
                    )}
                  >
                    {day.dayLabel}
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    {day.hourlyData.map((hour) => {
                      ...
                      return (
                        ...
                        >
                          <div className="text-xs font-medium">{hour.jam}</div>
